...
class RedisQueue(object):
    ...
    def rget(self, isBlocking=False, timeout=None): # 데이터 꺼내기
        if isBlocking:
    ...
    def rput(self, element): 
        self.rq.rpush(self.key, element) 

    def lget(self, isBlocking=False, timeout=None):
        if isBlocking:
            element = self.rq.blpop(self.key, timeout=timeout) 
            element = element[1] 
        else:
            element = self.rq.lpop(self.key)
        return element 
