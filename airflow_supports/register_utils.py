...
class AirflowRegisterer:
    ...
    def __init__(self, user_input):
        ...
        self.exp_yaml_path = "../solution/experimental_plan.yaml"
    ...
    def set_yaml(self, pipeline, version=VERSION):
        ...
        self.sm_yaml['metadata_version'] = version
        self.sm_yaml['name'] = ''
    ...
    def read_yaml(self, yaml_file_path):
        ...
        if  'solution_metadata' in yaml_file_path:
            self.sm_yaml = data
    ...
    def s3_upload_icon(self, icon_prefix:Optional[str]=''):
        # inner func.
        ...
        self.ICON_FILE = 'image/icon.svg'
        data_path = f'./{self.ICON_FILE}'
        if not icon_prefix:
            s3_file_path = icon_prefix.strip('/') + '/' + self.ICON_FILE
        else:
...
#---------------------------------------------------#
# methods for AIRFLOW - 기존 RegisterUtils 에 이어서 #
#---------------------------------------------------#
    def set_ecr_directly(self, ecr_uri, tags=[], docker=False):
        self.docker = docker
        self.ecr = ecr_uri
        self.ecr_url = self.ecr.split("/")[0]
        # FIXME 마지막에 붙는 container 이름은 solution_name 과 같게 
        ...
        # [중요] container uri 는 magna-ws 말고 magna 같은 식으로 쓴다 (231207 임현수C)
        ecr_scope = self.URI_SCOPE.split('-')[0] # magna-ws --> magna
        self.ecr_repo = "/".join(self.ecr.split("/")[1:])
        self.ecr_full_url = self.ecr_url + '/' + self.ecr_repo 
        if self.docker == True:
            run = 'docker'
        else:
        ...
        try:
            ...
            print_color('skip creating ecr : check authority or ecr-uri correctly', color='yellow')
        

    def set_s3_directly(self, bucket_name, prefix):
        self.bucket_name = bucket_name

        # workspace 내 aws role 부여 되어있어 s3내 인증 절차 불필요
        self.s3 = boto3.client('s3')
        # self.s3 는 s3 prefix로 사용하고 있음
        # 참고 : f'ai-solutions/{self.solution_name}/v{VERSION}/{self.pipeline}/{contents}' 처럼 양쪽 / strip
        self.s3_path = prefix.strip('/')
        
        if isinstance(self.s3, botocore.client.BaseClient):       
            print_color(f"\n[INFO] AWS S3 access check: OK", color="green")
        else: 
            raise ValueError(f"\n[ERROR] AWS S3 access check: Fail")

    def set_airflow_model_url(self, train_artifact_s3_url):
        if self.pipeline == "train":
            raise ValueError("not supported")
        else:
            self.sm_yaml["pipeline"][1].update(
                {'model_uri': 
                     train_artifact_s3_url
                }
            )
        self.save_yaml()

    def set_airflow_artifacts_uri(self, artifacts_s3_url):
        try: 
            data = {'artifact_uri': artifacts_s3_url}
            if self.pipeline == 'train':
                self.sm_yaml['pipeline'][0].update(data)
            elif self.pipeline =='inference':
                self.sm_yaml['pipeline'][1].update(data)
    ...
    def s3_upload_model_for_inference(self):
        def s3_process(s3, bucket_name, data_path, local_folder, s3_path, delete=True):
            if delete == True: 
                objects_to_delete = s3.list_objects(Bucket=bucket_name, Prefix=s3_path)
                if 'Contents' in objects_to_delete:
        ...
        if self.pipeline == 'train':
            raise ValueError("in train pipeline, model will be saved in artifacts")
